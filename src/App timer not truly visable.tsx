...
interface Todo {
  ...
  createdAt: number; totalTimeMs: number;
}
...
// ─── SubtaskItem ──────────────────────────────────────────────────────────────
function SubtaskItem({ subtask, onToggle, onDelete, onSave }: {
  subtask: Subtask;
...
function SortableItem({
  ...
  const addSubtask = () => {
    ...
    onAddSubtask(todo.id, { id: crypto.randomUUID(), text: newSubText.trim(), completed: false, priority: newSubPriority, dueDate: newSubDue, createdAt: Date.now() });
...
export default function App() {
  ...
  const addTodo = () => {
    if (!text.trim()) return;
    const t: Todo = { id: crypto.randomUUID(), text: text.trim(), notes, completed: false, priority, dueDate, tags: [...tags], subtasks: [], recur, lastReset: today(), createdAt: Date.now(), totalTimeMs: 0 };
  ...
  const brainDumpCapture = (items: string[]) => {
    const newTodos: Todo[] = items.map(text => ({ id: crypto.randomUUID(), text, notes: "", completed: false, priority: "medium", dueDate: "", tags: [], subtasks: [], recur: "none", lastReset: today(), createdAt: Date.now(), totalTimeMs: 0 }));
